...
## 25. The menu design document is a canvas export, so it was ported rather than converted
...
## 26. The chef answers in three channels at once, and interrupts in only one voice
...
## 27. The deployed cook flow has no source in this repository, so it was rebuilt rather than restyled
...
## 28. The chef's bank can only say what was written in advance, so the headset never heard the model
